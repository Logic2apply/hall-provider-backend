...
def viewHalls(request):
    if request.method == "POST":
        ...
        bookingsOnTime = Booking.objects.filter(
            startTime__gte=startTime, endTime__lte=endTime, dateOfEvent=eventdate, approvedStatus = True
        ).all()

        print(bookingsOnTime)
        hallsAvaliable = Hall.objects.exclude(id__in=[i.hall.id for i in bookingsOnTime]).all()
        ...
        print(hallsAvaliable)
        return render(request, 'home/viewHalls.html', {'hallsAvailable':hallsAvaliable})
...
def addBooking(request):
    if request.method == 'POST':
        try:
            ...
            startTime = request.POST.get('start_time')
            endTime = request.POST.get('end_time')
            bookingsOnTime = bookingsOnTime = Booking.objects.filter(startTime__gte=startTime, endTime__lte=endTime, dateOfEvent=dateOfEvent, approvedStatus = True).all()
            for booking in bookingsOnTime:
                ...
                if datetime.strptime(startTime, "%H:%M") >= datetime.strptime(endTime, "%H:%M"):
                    ...
                    return redirect("addBooking")
            booking = Booking(hall=hall, requestor=requestor, provider=provider, dean=dean, club=club, department=department, eventName=eventName, eventDescription=eventDescription, dateOfEvent=dateOfEvent, startTime=startTime, endTime=endTime)
